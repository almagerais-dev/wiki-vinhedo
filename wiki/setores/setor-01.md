---
tipo: setor
titulo: Setor 01
codigo: setor-01
local: macaia
ruas: ["1-47"]
variedades: [sauvignon-blanc]
quadras: []
area_ha: 1.0585
geometria_id: 5
nome_na_fonte: null
tags: [mapa-plantio]
fontes:
  - raw/mapa-plantio/2026-08-29-mapa-de-plantio-alma-gerais-vivert.json
  - raw/mapa-plantio/2026-08-29-plantio-vivert.geojson
  - raw/mapa-plantio/2026-08-29-confirmacao-doze-setores.md
  - raw/mapa-plantio/2026-08-29-mapa-rotulado-setores.png
atualizado_em: 2026-09-19
---

# Setor 01

## Cadastro

- **Local:** [[macaia]]
- **Identificação na fonte:** parte `5` do MultiPolygon `Plantio Vivert`, ligada a
  `setor-01` pela imagem rotulada de 2026-08-29. As partes não carregam atributo e a ordem
  delas no arquivo **não** acompanha a numeração dos setores.
- **Área:** 1,0585 ha, geodésica WGS84 calculada sobre a parte `5`; a coleção de 2026-08-29
  não traz `area_ha`.
- **Geometria vigente:** parte `5` do MultiPolygon em
  `raw/mapa-plantio/2026-08-29-plantio-vivert.geojson`.
- **Ruas:** 1 a 47.
- **Variedades:** [[sauvignon-blanc]].
- **Quadras relacionadas:** a fonte de plantio não relaciona quadras.
- **Plantas:** 4.232 (mapa de plantio de 2026-08-29).
- **Produção anual esperada:** 5.924,8 kg (valor de planejamento da fonte; não é colheita observada).

## Plantio

Blocos do mapa de plantio `Alma Gerais Vivert` (2026-08-29). Cada linha da fonte é um trecho de
ruas dentro do setor, não uma quadra. `kg/planta` e a produção anual são valores **esperados** da
fonte.

| Ruas | Variedade | Plantas | Ano de plantio | Clone | Porta-enxerto | kg/planta esperado | Produção anual esperada (kg) |
|---|---|---:|---|---|---|---:|---:|
| 1 a 47 | [[sauvignon-blanc]] | 4.232 | 2020 | 242 | 1103P | 1,4 | 5.924,8 |

## Históricos mensais

Do mais recente para o mais antigo:

- [[2026-09-setor-01]] — pulverizações de gema algodão, com Zorvec e com a calda de produtos novos
  de 2026-09-18, início da brotação e o primeiro desbrote.
- [[2026-08-setor-01]] — segunda fertirrigação com MAP.

## Correlações e hipóteses

- [[conferencia-das-aplicacoes-de-cercobin-2026]] — conferência da pulverização de Cercobin deste setor contra a bula do produto (dose, vazão e época).
- [[identidade-dos-produtos-da-pulverizacao-de-2026-09-18]] — hipótese sobre a identidade dos produtos da calda de 2026-09-18.

## Fontes cadastrais

- `raw/mapa-plantio/2026-08-29-mapa-de-plantio-alma-gerais-vivert.json`
- `raw/mapa-plantio/2026-08-29-plantio-vivert.geojson`
- `raw/mapa-plantio/2026-08-29-confirmacao-doze-setores.md`
- `raw/mapa-plantio/2026-08-29-mapa-rotulado-setores.png`
