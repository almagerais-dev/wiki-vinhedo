---
tipo: setor
titulo: Setor 09
codigo: setor-09
local: macaia
ruas: ["1-9", "10-41", "42-71"]
variedades: [piwis, cabernet-franc]
quadras: []
area_ha: 1.3431
geometria_id: 10
nome_na_fonte: null
tags: [mapa-plantio]
fontes:
  - raw/mapa-plantio/2026-08-29-mapa-de-plantio-alma-gerais-vivert.json
  - raw/mapa-plantio/2026-08-29-plantio-vivert.geojson
  - raw/mapa-plantio/2026-08-29-confirmacao-doze-setores.md
  - raw/mapa-plantio/2026-08-29-mapa-rotulado-setores.png
atualizado_em: 2026-09-16
---

# Setor 09

## Cadastro

- **Local:** [[macaia]]
- **Identificação na fonte:** parte `10` do MultiPolygon `Plantio Vivert`, ligada a
  `setor-09` pela imagem rotulada de 2026-08-29. As partes não carregam atributo e a ordem
  delas no arquivo **não** acompanha a numeração dos setores.
- **Área:** 1,3431 ha, geodésica WGS84 calculada sobre a parte `10`; a coleção de 2026-08-29
  não traz `area_ha`.
- **Geometria vigente:** parte `10` do MultiPolygon em
  `raw/mapa-plantio/2026-08-29-plantio-vivert.geojson`.
- **Ruas:** 1 a 9, 10 a 41, 42 a 71.
- **Variedades:** [[piwis]], [[cabernet-franc]].
- **Quadras relacionadas:** a fonte de plantio não relaciona quadras.
- **Plantas:** 2.250 (mapa de plantio de 2026-08-29).
- **Produção anual esperada:** 5.625 kg (valor de planejamento da fonte; não é colheita observada).

## Plantio

Blocos do mapa de plantio `Alma Gerais Vivert` (2026-08-29). Cada linha da fonte é um trecho de
ruas dentro do setor, não uma quadra. `kg/planta` e a produção anual são valores **esperados** da
fonte.

| Ruas | Variedade | Plantas | Ano de plantio | Clone | Porta-enxerto | kg/planta esperado | Produção anual esperada (kg) |
|---|---|---:|---|---|---|---:|---:|
| 1 a 9 | [[piwis]] | 500 | 2024 | — | — | 2,5 | 1.250 |
| 10 a 41 | sem plantio | 0 | — | — | — | — | — |
| 42 a 71 | [[cabernet-franc]] | 1.750 | 2023 | 214 | IAC766 | 2,5 | 4.375 |

## Históricos mensais

Do mais recente para o mais antigo:

- [[2026-09-setor-09]] — primeira fertirrigação ancorada, primeira pulverização e o início da
  brotação; a fertirrigação de `689a0544` segue sem data.
- [[2026-08-setor-09]] — preparo de poda com Ethrel e poda.

## Correlações e hipóteses

- [[conferencia-das-aplicacoes-de-cercobin-2026]] — conferência da pulverização de Cercobin deste setor contra a bula do produto (dose, vazão e época).

## Fontes cadastrais

- `raw/mapa-plantio/2026-08-29-mapa-de-plantio-alma-gerais-vivert.json`
- `raw/mapa-plantio/2026-08-29-plantio-vivert.geojson`
- `raw/mapa-plantio/2026-08-29-confirmacao-doze-setores.md`
- `raw/mapa-plantio/2026-08-29-mapa-rotulado-setores.png`
