---
tipo: evento
categoria: fitossanidade
data: 2026-09-16
ingerido_em: 2026-09-19
local: macaia
setores: [setor-02]
ruas: []
quadras: []
variedades: []
safra: 2027
ciclo: null
estagio_fenologico: null
tags: [pulverizacao, cercobin, absolut-fix]
fontes: [raw/registros-manejo/2026-09-18-macaia-setor-02-fertirrigacao-e-pulverizacao.md]
granola_id: 73ef1fb1-0f27-439d-87de-6f65edfc9d21
atualizado_em: 2026-09-19
---

# 2026-09-16 — Pulverização com Cercobin no setor 02

## O que aconteceu (fato)

Pulverização no setor 02 em 2026-09-16, com 600 g de Cercobin e 600 ml de Absolut Fix, em 200 L de
calda por hectare. A data é declarada como "feito no dia 16, do 9 de 2026", forma inequívoca.

É a primeira pulverização registrada para o setor 02 desde 2026-09-02
([[2026-09-02-macaia-setor-01-pulverizacao-gema-algodao]] cobre o setor 01, não este). A calda
repete a dupla Cercobin + Absolut Fix nas mesmas doses das pulverizações de 2026-09-02 a
2026-09-10, sem o inseticida daquelas caldas, e não adota o Zorvec que entrou em 2026-09-14 nos
setores 01, 05, 06, 07 e 08. A mesma calda, na mesma data, foi aplicada nos setores 04
([[2026-09-16-macaia-setor-04-pulverizacao-cercobin]]) e 10
([[2026-09-16-macaia-setor-10-pulverizacao-cercobin]]).

A fonte não diz se as doses de produto são por hectare ou por tanque de 200 L, nem declara alvo ou
área tratada.

A **fertirrigação** relatada na mesma gravação não gerou evento: sua única data é "16 de novembro de
2026", posterior à própria gravação. Ver a `pendencia` de 2026-09-19 em `log.md`.

## Contexto

- **Safra:** [[safra-2027]]
- **Local:** [[macaia]]
- **Setor(es):** [[setor-02]]
- **Rua(s):** —
- **Quadra(s) / variedade(s):** — / —
- **Estágio fenológico:** não declarado. Nenhuma gravação registra o início da brotação do setor 02,
  e a operação não define o estágio; o campo fica nulo em vez de deduzido.
- **Origem Granola:** ver `granola_id` e `fontes` no frontmatter.

## Interpretações relacionadas

Conferência da dose, da vazão e da época desta aplicação contra a bula do [[cercobin-875-wg]]: [[conferencia-das-aplicacoes-de-cercobin-2026]].

Ver [[fitossanidade]] para o conjunto das pulverizações do ciclo.
