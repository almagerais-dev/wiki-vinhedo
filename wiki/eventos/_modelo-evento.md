---
tipo: evento
categoria: <clima | manejo | analise | fitossanidade | qualidade | colheita | observacao>
data: <AAAA-MM-DD>
local: macaia
quadras: []
variedades: []
safra: <AAAA>
estagio_fenologico: <dormencia | brotacao | floracao | pegamento | crescimento-baga | veraison | maturacao | colheita | pos-colheita>
tags: []
fontes: []
atualizado_em: <AAAA-MM-DD>
---

# <AAAA-MM-DD> — <descrição curta do fato>

## O que aconteceu (fato)

_Descrição objetiva do fato observado/medido. **Sem** especular causa aqui._

## Contexto

- **Safra:** [[safra-<AAAA>]]
- **Local:** [[<local>]]
- **Quadra(s) / variedade(s):** [[<quadra>]] / [[<variedade>]]
- **Estágio fenológico:** ver frontmatter.

## Interpretações relacionadas

_Links para `wiki/correlacoes/` e `wiki/hipoteses/` (as suposições vivem lá, não aqui)._
