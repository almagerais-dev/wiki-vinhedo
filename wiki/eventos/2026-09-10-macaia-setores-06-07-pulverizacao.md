---
tipo: evento
categoria: fitossanidade
data: 2026-09-10
ingerido_em: 2026-09-12
local: macaia
setores: [setor-06, setor-07]
ruas: []
quadras: []
variedades: []
safra: 2027
ciclo: null
estagio_fenologico: null
tags: [pulverizacao, cercobin, absolut-fix]
fontes: [raw/registros-manejo/2026-09-11-macaia-setores-06-07-pulverizacao.md]
granola_id: 891dc4b7-bce7-4c90-aa43-bad711d87ff5
atualizado_em: 2026-09-12
---

# 2026-09-10 — Pulverização nos setores 06 e 07

## O que aconteceu (fato)

Pulverização nos setores 06 e 07 com 600 g de Cercobin e 600 ml de Absoluto Fix, em 200 L de calda
por hectare, na data "dia 10 do 9 de 2026".

É a primeira pulverização com evento próprio nestes setores. A de 2026-09-02, relatada na mesma
gravação que a fertirrigação de `001b16e3`, ficou sem evento por data impossível e a pendência
segue aberta desde 2026-09-04.

A fonte abre chamando a operação de "Ovelhização", palavra que o título do Granola repete e que não
corresponde a nenhuma operação do vinhedo. A wiki a classifica como pulverização pelo que a própria
fonte descreve — produtos, volume de calda por hectare e a mesma estrutura das outras pulverizações
da sequência — e registra a grafia original aqui, sem atribuir alvo nem estágio que a fonte não
declara.

A fonte não diz se as doses de produto são por hectare ou por tanque de 200 L, nem informa a área
tratada.

## Contexto

- **Safra:** [[safra-2027]]
- **Local:** [[macaia]]
- **Setor(es):** [[setor-06]], [[setor-07]]
- **Rua(s):** —
- **Quadra(s) / variedade(s):** — / —
- **Estágio fenológico:** a fonte não declara estágio para esta operação; campo nulo.
- **Origem Granola:** ver `granola_id` e `fontes` no frontmatter.

## Interpretações relacionadas

Conferência da dose, da vazão e da época desta aplicação contra a bula do [[cercobin-875-wg]]: [[conferencia-das-aplicacoes-de-cercobin-2026]].

Ver [[fitossanidade]] para o conjunto das pulverizações.
